...
This directory stores the Baseport documentation, served by [Bark](https://github.com/melosso/bark). As most of these are generated by an LLM (as we have not have had the capability to co-author these pages), this README-file can be seen as an agentic instruction. Pages are plain markdown under `docs/`, with site configuration in `docs/config.json`. There is nothing to install to write a page.
...
A few habits worth avoiding, because they read as clever instead of useful:
...
- Closing aphorisms. If a sentence is there for rhythm instead of information, cut it.
...
Copy also follows the Bark tone guidelines: suggestive and explanatory instead of commanding, no em or en dashes, and loud callouts reserved for genuine edge cases. A quick pre-publish check for aggressive phrasing:
