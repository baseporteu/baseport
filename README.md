...
Both put a `baseport` command on your PATH, so `baseport --urls http://localhost:5000` starts it, `baseport logs` follows the log, and `baseport update` upgrades it in place. Releases ship `linux-x64` and `win-x64`.
