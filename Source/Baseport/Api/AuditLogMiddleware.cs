...
public static class AuditLogMiddleware
{
    ...
    public static IApplicationBuilder UseAuditLog(this IApplicationBuilder app) =>
        app.Use(async (context, next) =>
        {
            ...
            // Set by the endpoint while it ran, so one row carries both what was requested and what it meant. A sign-in that failed has no session to name the caller by, and an OpenID Connect return is a GET that would otherwise never be recorded at all.
