...
// The console shows who is signed in; the username is the seed, so an account looks the same
// on every screen and after every restart without a column to hold it.
public static class Avatars
{
    // Parsing a style reads and validates its schema, so it happens once.
    ...
    // Pastels, so a full-bleed avatar sits under dark text in either theme.
    private static readonly JsonArray Backgrounds =
        new("ffe3ea", "e3edff", "e2f5e9", "fdf1d4", "efe6ff");

    // An account list is short; the bound only stops a rename loop growing this without end.
    private const int MaxCached = 256;
    private static readonly ConcurrentDictionary<string, string> Cache = new(StringComparer.Ordinal);

    // Null rather than a throw: an avatar is decoration, and it must not be able to take the console down.
