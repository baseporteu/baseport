...
const ui = (() => {
  ...
  const ICONS = {
    ...
    error:
      '<svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M18 6 6 18M6 6l12 12"/></svg>',
  ...
  async function send(url, { method = 'GET', body, success, failure } = {}) {
    ...
    return handle(await fetch(url, init), { success, failure });
  }
  ...
  if (window.matchMedia) {
    ...
    const follow = (ev) => {
      ...
      if (!stored) setTheme(ev.matches ? 'dark' : 'light', { remember: false });
    };
  ...
  function field(label, { id, type = 'text', value = '', placeholder = '', help = '', options, mono, rows = 3, name } = {}) {
    const wrap = el('label', 'field');
    wrap.append(el('span', 'field-label-text', { textContent: label }));

    let input;
    if (type === 'textarea') {
      input = el('textarea', 'input' + (mono ? ' mono' : ''), { rows });
    } else if (type === 'select') {
      input = el('select', 'input');
      (options || []).forEach(([v, l]) => input.append(el('option', null, { value: v, textContent: l })));
    } else {
      input = el('input', type === 'checkbox' ? '' : 'input' + (mono ? ' mono' : ''), { type });
    }
    if (id) input.id = id;
    ...
    wrap.append(input);
    if (help) wrap.append(el('span', 'field-help', { textContent: help }));
    wrap.control = input;
    return wrap;
  }
  ...
  function combobox(label, { id, value = '', valueLabel = '', placeholder = '', help = '', fetchOptions, browseAll = false } = {}) {
    const wrap = el('label', 'field combobox');
    if (label) wrap.append(el('span', 'field-label-text', { textContent: label }));

    const box = el('div', 'combobox-box');
    const search = el('input', 'input', { type: 'text', placeholder, autocomplete: 'off', value: valueLabel });
    const hidden = el('input', null, { type: 'hidden', value });
    if (id) hidden.id = id;
  ...
  function button(label, onClick, { variant = '', size = '', type = 'button', title } = {}) {
    const b = el('button', ['btn', variant, size].filter(Boolean).join(' '), { type, textContent: label });
  ...
  function sheet(title, bodyEl, actionsEl) {
    closeSheet();
    sheetDirty = false;
    const overlay = el('div', 'overlay', { id: 'sheetOverlay' });
    overlay.onclick = attemptCloseSheet;

    const panel = el('div', 'sheet');
    const head = el('div', 'sheet-head');
    head.append(
      el('h3', null, { textContent: title }),
      button('×', attemptCloseSheet, { variant: 'btn-ghost', size: 'btn-sm' }),
    );
  ...
  function ask({ title, label, value = '', placeholder = '', confirmLabel = 'Save' }) {
    return new Promise((resolve) => {
      ...
      actions.append(
        button('Cancel', () => done(null), { variant: 'btn-outline' }),
  ...
  function confirm({ title, message, confirmLabel = 'Confirm', cancelLabel = 'Cancel', danger = false }) {
    return new Promise((resolve) => {
      const body = el('p', 'muted', { textContent: message });
      const actions = el('div', 'row');
      actions.append(
        button(
          cancelLabel,
