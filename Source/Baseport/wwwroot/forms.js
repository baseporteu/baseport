...
const ACTION_HINTS = {
  ...
  lookup:
    'Finds one record by an identifier the visitor already has. An unknown value returns the same not-found message every time.',
...
async function renderFormsList() {
  ...
  const params = new URLSearchParams({ sort: sort.key, order: sort.dir });
...
function deleteForm(id) {
  ...
  openModal({
    ...
    danger: true,
    onConfirm: async () => {
      const res = await fetch(`/api/_admin/forms/${id}`, { method: 'DELETE' });
      if (!(await ui.handle(res, { success: 'Form deleted.', failure: 'The form could not be deleted.' }))) return;
...
async function editForm(id) {
  const f = await fetch(`/api/_admin/forms/${id}`)
    .then((r) => r.json())
    .catch(() => null);
  if (!f || !f.id) return navigate('/forms', { replace: true });
...
function onActionsChange() {
  ...
  formConfigDraft = { ...formConfigDraft, ...collectConfig() };
...
function applyKindConfig(cfg) {
  ...
  listColumns = (cfg.columns || []).map((name) => ({ name, render: renderers[name] || '' }));
  listActions = (cfg.actions || []).map((a) => ({ label: a.label || '', hrefExpr: a.hrefExpr || '' }));
...
function addListAction() {
  listActions.push({ label: '', hrefExpr: '' });
  renderListActions();
}
...
function insertColumn(name, at) {
  ...
  listColumns.splice(at === undefined ? listColumns.length : at, 0, { name, render: '' });
  renderListBuilder();
}
...
function addListFilter() {
  ...
  renderListFilters(collectListFilters().concat({ field: selectableFields()[0].name, op: 'eq', value: '' }));
}
...
function collectListFilters() {
  return Array.from(document.querySelectorAll('#listFilters .filter-row')).map((row) => {
    ...
    return { field: field.value, op: op.value, value: value.value };
  });
}
...
async function saveForm(btn) {
  await ui.busy(btn, async () => {
    const body = {
      ...
      configJson: JSON.stringify({ ...formConfigDraft, ...collectConfig() }),
...
function parseLayout(layoutJson) {
  try {
    ...
    if (Array.isArray(p))
      return {
        rows: p.map((r) => ({ t: 'row', cols: [{ t: 'col', w: 12, items: r.filter((x) => x !== 'spacer') }] })),
      };
  } catch (e) {}
  return { rows: [] };
}
...
function emptyCol() {
  return { t: 'col', w: 12, items: [] };
}

function addRow(type, atIndex) {
  let row;
  if (type === 'group') row = { t: 'group', title: 'Group', cols: [emptyCol()] };
  else if (type === 'row') row = { t: 'row', cols: [emptyCol()] };
  else if (type === 'subtotal') row = { t: 'subtotal', label: 'Total', expr: '', format: 'currency' };
  else if (type === 'button') row = { t: 'button', label: 'Button', action: 'submit' };
  if (!row) return;
...
function renderColumn(row, ri, col, ci) {
  ...
  col.items.forEach((item) => {
    ...
    chip.addEventListener('dragstart', (ev) => {
      ...
      ev.dataTransfer.setData('text/movefield', JSON.stringify({ row: ri, col: ci }));
    });
...
(function wireCanvasDrops() {
  ...
  canvas.addEventListener('drop', (ev) => {
    ...
    const field = ev.dataTransfer.getData('text/field');
    if (field) {
      layout.rows.push({ t: 'row', cols: [{ t: 'col', w: 12, items: [field] }] });
      renderCanvas();
    }
  });
})();
