...
function initSqlEditor() {
  ...
  sqlEditor.on('keyup', (cm, e) => {
    ...
    if (e.key === ' ' || e.key === '.') cm.showHint({ completeSingle: false });
  });
}
...
async function saveQuery() {
  ...
  const name = currentQueryId
    ? currentQueryName
    : await ui.ask({ title: 'Save query', label: 'Query name', placeholder: 'Field counts' });
  if (!name) return;
...
async function renameQuery(q) {
  const name = await ui.ask({ title: 'Rename query', label: 'Query name', value: q.name });
  if (!name || name === q.name) return;
  await ui.send(`/api/_admin/queries/${q.id}`, { method: 'PATCH', body: { name }, success: 'Query renamed.' });
...
function deleteQuery(q) {
  openModal({
    ...
    danger: true,
    onConfirm: async () => {
      await fetch(`/api/_admin/queries/${q.id}`, { method: 'DELETE' });
...
async function runSql() {
  ...
  const meta = await ui.fragment('sqlResult', '/api/_admin/fragments/sql', {
    body: { sql, queryId: currentQueryId || null },
