...
function sortState(listKey, defaultKey) {
  try {
    return JSON.parse(localStorage.getItem('bp.sort.' + listKey)) || { key: defaultKey, dir: 'asc' };
  } catch (e) {
...
function initSortableHeaders(headRowId, listKey, defaultKey, onChange) {
  ...
  headRow.querySelectorAll('th[data-sort]').forEach((th) => {
    ...
    th.onclick = () => {
      ...
      localStorage.setItem('bp.sort.' + listKey, JSON.stringify({ key, dir }));
      onChange(key, dir);
    };
  });
}
...
const ROUTES = [
  ...
  [
    ...
    (m) => ({ section: 'tables', id: m[1], view: m[2] === 'records' ? 'records' : 'builder' }),
  ],
  [/^\/forms(?:\/([\w-]+))?$/, (m) => ({ section: 'forms', id: m[1] })],
  [/^\/sql(?:\/([\w-]+))?$/, (m) => ({ section: 'sql', id: m[1] })],
  [/^\/settings(?:\/([\w-]+))?$/, (m) => ({ section: 'settings', id: m[1] || 'host' })],
  [/^\/(schema|auth|logs)$/, (m) => ({ section: m[1] })],
];
...
const SECTION_ROUTES = {
  ...
  tables: async (id) => {
    ...
    if (!table) return navigate('/tables', { replace: true });
    selectTable(table);
