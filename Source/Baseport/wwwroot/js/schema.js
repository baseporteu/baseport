...
async function loadSchema() {
  ...
  tables.forEach((t, i) => {
    const col = i % cols;
    const row = Math.floor(i / cols);
    schemaNodes[t.id] = { x: col * 280, y: rowY[row], w: 240, h: heights[i] };
    ...
    card.addEventListener('pointerdown', (ev) => {
      ...
      schemaDrag = { type: 'node', pid: t.id, sx: ev.clientX, sy: ev.clientY, ox: n.x, oy: n.y };
...
(function initSchemaCanvas() {
  ...
  canvas.addEventListener('pointerdown', (ev) => {
    ...
    ev.preventDefault();
    schemaDrag = { type: 'pan', sx: ev.clientX, sy: ev.clientY, ox: schemaPan.x, oy: schemaPan.y };
    document.addEventListener('pointermove', onSchemaDragMove);
    document.addEventListener('pointerup', onSchemaDragEnd);
  });
