...
function deleteRecord(rid) {
    openModal({
        ...
        danger: true,
        onConfirm: async () => {
            await fetch(`/api/_admin/tables/${currentTablePublicId}/records/${rid}`, {
                method: 'DELETE'
            });
            await loadRecords();
            await loadTables();
    ...
    fields.forEach((f) => {
        ...
        if (type === 'longtext') row = ui.field(label, {
            ...
            row = ui.field(label, {
                id,
                type: 'text',
                placeholder: opts.length ? opts.join(', ') : '',
                help: ((f.helpText ? f.helpText + ' ' : '') + 'Comma-separated.').trim()
            });
        } else if (type === 'file') {
            row = ui.field(label, {
                id,
...
async function submitNewRecord(inputs) {
    ...
    const created = await ui.handle(res, {
        success: 'Record created.',
        failure: 'Could not create the record.'
    });
    if (!created) return;
    closeSheet();
    await loadRecords();
...
async function saveTableSettings(btn) {
    ...
    await ui.busy(btn, async () => {
        ...
        saved = await ui.handle(res, {
            ...
            failure: 'Could not save the table settings.'
        });
        if (!saved) return;
        tableDirty = false;
        await loadTables();
